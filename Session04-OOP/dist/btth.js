"use strict";
class ToDo {
    constructor(_id, content, status = "false") {
        this._id = _id;
...
class ToDoListManager {
    ...
    updateToDo(index, content, status) {
        if (index <= 0 || index > this.toDoList.length) {
    ...
    listToDo() {
        ...
        this.toDoList.forEach((todo, index) => console.log(`${index}: [${todo.id}] ${todo.content} - ${todo.status}`));
    }
}
class Main {
    static start() {
        ...
        let isRunning = true;
        while (isRunning) {
            input = prompt("Chọn thao tác: (A) Thêm ToDo, (R) Xóa ToDo, (U) Cập nhật, (S) Sắp xếp, (F) Tìm kiếm, (L) Liệt kê, (E) Thoát");
            switch (input === null || input === void 0 ? void 0 : input.toUpperCase()) {
                case "A":
                ...
                case "U":
                    ...
                    if (newContent)
                        manager.updateToDo(updateIndex, newContent, newStatus);
                    break;
                case "S":
                    manager.sortToDo();
                    break;
                case "F":
                    ...
                    if (searchContent)
                        manager.findToDo(searchContent);
                    break;
                case "L":
                    manager.listToDo();
                    break;
